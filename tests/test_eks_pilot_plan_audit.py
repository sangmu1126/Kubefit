...
from safety.eks_pilot_plan_audit import EksPilotPlanError, audit_creation_plan
...
def test_rejects_broad_or_non_ipv4_endpoint(cidr: str) -> None:
    plan = _plan()
    plan["resource_changes"][5]["change"]["after"]["vpc_config"][0][
        "public_access_cidrs"
    ] = [cidr]
...
def test_rejects_changed_node_count() -> None:
    plan = _plan()
    plan["resource_changes"][6]["change"]["after"]["scaling_config"][0][
        "desired_size"
    ] = 3
