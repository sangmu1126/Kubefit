...
| Claim | Evidence |
|---|---|
| Current source contracts are verified | 619 Python tests and Ruff at the latest resource-change gate verification ([record 0110](docs/devlog/0110-resource-change-throttling-gate.md)) |
...
An earlier generic live run reduced requests from CPU `1000m` to `500m` and memory
...
A later disposable EKS pilot collected 100,503 fixed-load requests with zero errors and
produced a read-only recommendation. The cluster and all Terraform-managed resources
were destroyed afterward. This does **not** establish an AWS bill reduction
([record 0108](docs/devlog/0108-third-eks-pilot-complete.md)). A subsequent local
kind before/after trial exposed CPU throttling despite an older performance-only PASS.
The new resource-change gate now requires UID-bound Prometheus throttling evidence;
its live verification recorded a FAIL for the aggressive candidate
([record 0110](docs/devlog/0110-resource-change-throttling-gate.md)).
...
- No live PodKill recovery result exists: the tested generic Pair correctly failed its
  prerequisite gate.
- The EKS pilot was read-only; no measured AWS bill reduction or real-user workload
